...
size_t Graph::toHash() {
  ...
  for (boost::tie(it, endr) = boost::vertices(graph); it != endr; ++it) {
    vp[*it].index = i;
    i++;
  ...
  while (pq.size() != 0) {
    ...
    adjacency_iter ai, a_end;
    boost::tie(ai, a_end) = adjacent_vertices(*toVert[u.index], graph);
    std::vector<std::string> adj_labels;
    std::unordered_map<std::string, adjacency_iter> strToVert;
    for (; ai != a_end; ai++) {
      // std::cout << "--------------------------------" << std::endl;
      ...
      std::string temp = vp[*ai].label + std::to_string(vp[*ai].index);
      adj_labels.push_back(temp);
      strToVert[temp] = ai;
    }
    std::sort(adj_labels.begin(), adj_labels.end(), compareFunction);
    for (auto &label : adj_labels) {
      pq.push(vp[*strToVert[label]]);
    }
  }
  return strHasher(ss.str());
}
